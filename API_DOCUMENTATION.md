...
JWT role claims use two concepts:
...
`active_role` is the account mode selected by the auth endpoint used to login, for example `cleaner` when using `/auth/cleaner/signin`. `roles` contains every role assigned to the user. `role` is a backward-compatible alias for `active_role`; new frontend code should prefer `active_role`.
...
All auth and profile phone fields expect a 10-digit Indian mobile number only:
...
Do not send a country code from the frontend. OTP delivery uses SMSCountry and the backend adds the India country code only inside `services/sms_service.py`.

The SMS message body is generated by the backend. Frontend should never build or submit the OTP SMS text. Current SMSCountry-approved admin OTP body:

```txt
User Admin login OTP is <otp> - SMSCNT
```
...
Required fields: `address_line1`, `latitude`, `longitude`.

### UpdateAddressRequest

Same fields as `CreateAddressRequest`, but all fields are optional. If the frontend sends latitude or longitude during update, it must send both.
...
Required field: `vehicle_type`.

### UpdateCustomerVehicleRequest

Same fields as `CreateCustomerVehicleRequest`, but all fields are optional.
...
Use either `address_id`, `address`, or omit both to use the customer's default address. `vehicle_id` is optional; if omitted, the backend uses the customer's default vehicle when one exists.
...
Payment fields are optional on completion. The separate payment collection API can also be used after completion.
...
The split total must equal the collected amount.
...
`booking_id` appears in both the path and body and must match.
...
Rating must be from `1` to `5` and can have at most one decimal place.
...
Role-specific signup and signin responses also include `account_type` and `user`. Cleaner auth responses also include `cleaner`. The JWT payload includes `active_role`, `roles`, `type`, `exp`, `iat`, and `jti`; access tokens have `type="access"` and refresh tokens have `type="refresh"`.
...
Returned inside booking responses.
...
### OTP Send And Verify Examples

Send OTP through SMSCountry:
...
Verify the OTP during signup or signin:
